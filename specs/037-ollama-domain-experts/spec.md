...
As a developer using Kiro IDE, I want the same ollama-experts MCP pointed at my local AI host (your-ollama-host:11434) so that I can iterate on the demo workflow — testing config generation, validating GraphQL queries — without burning Frontier tokens.
...
1. **Given** the ollama-experts MCP configured in `.kiro/settings/mcp.json` pointing to your-ollama-host:11434, **When** I ask it to generate an FRR config from device data, **Then** it delegates to the local model and returns structured config.
...
- **FR-008**: System MUST support `OLLAMA_BASE_URL` env var for remote Ollama instances (default: http://your-ollama-host:11434).
...
- User has Ollama running on your-ollama-host:11434 with `qwen2.5-coder:32b-instruct-q8_0` available (confirmed from earlier inventory).
